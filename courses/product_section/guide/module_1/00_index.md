# 📊 Модуль 1. Продуктовые метрики

Цель модуля — научиться **корректно выбирать и интерпретировать продуктовые метрики**,
не делать типовых ошибок и уверенно использовать метрики в кейсах и в работе.

---

## Как проходить модуль

- Задача — **понять логику**, а не запомнить определения.
- ★ — необязательные блоки для углубления. Можно пропустить при первом прохождении.
- Если можешь объяснить идею словами — значит, понял.

---

## 📦 Блок А. Продуктовые метрики

Основной поток. Все страницы обязательны.

| # | Тема | Файл |
|---|---|---|
| 1.0 | Введение: как работать с продуктовыми метриками | [1_0_intro.md](1_0_intro.md) |
| 1.1 | DAU / WAU / MAU — метрики аудитории | [1_1_dau_wau_mau.md](1_1_dau_wau_mau.md) |
| 1.2 | Retention, PMF и Aha-moment | [1_2_retention.md](1_2_retention.md) |
| 1.3 | Практика: Retention | [1_3_practice_retention.md](1_3_practice_retention.md) |
| 1.4 | Конверсия и воронки | [1_4_conversion_funnels.md](1_4_conversion_funnels.md) |
| 1.5 | Практика: Воронки | [1_5_practice_funnels.md](1_5_practice_funnels.md) |
| 1.6 | Метрики монетизации | [1_6_monetization.md](1_6_monetization.md) |
| 1.7 | Дерево продуктовых метрик | [1_7_metrics_tree.md](1_7_metrics_tree.md) |
| 1.8 | Практика: Монетизация | [1_8_practice_monetization.md](1_8_practice_monetization.md) |
| 1.9 | Unit-экономика | [1_9_unit_economics.md](1_9_unit_economics.md) |
| 1.10 | Практика: Unit-экономика | [1_10_practice_unit.md](1_10_practice_unit.md) |

★ | Метрики качества поиска и рекомендаций | [1_6b_search_recs.md](1_6b_search_recs.md) |

---

## 🧠 Блок Б. Аналитическое мышление

Учишься видеть ловушки в данных и не делать ложных выводов.

| # | Тема | Файл |
|---|---|---|
| 1.11 | Когортный анализ | [1_11_cohort_analysis.md](1_11_cohort_analysis.md) |
| 1.12 | Практика: Когорты | [1_12_practice_cohorts.md](1_12_practice_cohorts.md) |
| 1.13 | Среднее vs медиана | [1_13_mean_vs_median.md](1_13_mean_vs_median.md) |
| 1.14 | Корреляция ≠ причинность | [1_14_correlation.md](1_14_correlation.md) |
| 1.15 | Парадокс Симпсона | [1_15_simpsons_paradox.md](1_15_simpsons_paradox.md) |

★ | Статистическая гигиена | [1_15_statistical_hygiene.md](1_15_statistical_hygiene.md) |

---

## 🔢 Финальная практика

| # | Тема | Файл |
|---|---|---|
| 1.16 | Чек-лист модуля | [1_16_checklist.md](1_16_checklist.md) |
| 1.17 | Excel-практикум: три бизнеса | [1_17_excel_practice.md](1_17_excel_practice.md) |

---

➡️ Начни с [1.0 Введения](1_0_intro.md)
