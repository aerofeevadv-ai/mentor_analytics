...
> ⚠️ **Деление на ноль:** в pandas `x / 0` даёт `inf`, а не ошибку — и inf молча ломает средние. После делений проверяй: `np.isinf(df["cpc"]).sum()`, заменяй `df.replace([np.inf, -np.inf], np.nan)`.
...
7. Топ-5 кампаний по ROMI среди тех, у кого spend > 10 000 (почему фильтр важен — объясни: ROMI на копеечных тратах нерепрезентативен).
